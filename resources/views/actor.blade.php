...
<div class="tv-info border-b border-gray-800 flex">
    <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
        <div class="flex-none">
            <img src="{{asset('https://image.tmdb.org/t/p/w500/' . $actors['profile_path'])}}" alt="poster" class="lg:w-64">
             <ul class="flex items-center mt-4">

                    @if ($actors['external_ids']['facebook_id'])
                        <li>
                            <a href="{{asset('https://www.facebook.com/' . $actors['external_ids']['facebook_id'])}}" title="Facebook">
                    ...
                    @if ($actors['external_ids']['instagram_id'])
                        <li class="ml-6">
                            <a href="{{asset('https://www.instagarm.com/' . $actors['external_ids']['instagram_id'])}}" title="Instagram">
                    ...
                    @if ($actors['external_ids']['twitter_id'])
                        <li class="ml-6">
                            <a href="{{asset('https://www.twitter.com/' . $actors['external_ids']['twitter_id'])}}" title="Twitter">
                    ...
                    @if ($actors['homepage'])
                        ...
                        <li class="ml-6">
                            <a href="{{ $actors['homepage'] }}" title="Website">
        ...
        <div class="md:ml-24">
                ...
                <div class="flex flex-wrap items-center text-gray-400 text-sm">
                    ...
                    <span class="ml-2">{{ $actors['birthday'] }} ({{\carbon\carbon::parse($actors['birthday'])->age}} years old) in {{ $actors['place_of_birth'] }}</span>
                </div>
                <div>
                    <div class='mt-4'>
                        <h2><b>Biography</b></h2>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
                    @foreach ($actors['credits']['crew'] as $crew)
                        @if ($loop->index <10)
                            <div class="mt-4">
                                <a href="{{route('movies.show', $crew['id'])}}"><img src="{{asset('https://image.tmdb.org/t/p/w500/' . $crew['poster_path'])}}" alt="poster" class="hover:opacity-75 transition ease-in-out duration-150"></a>
                                <a href="{{route('movies.show', $crew['id'])}}" class="text-sm leading-normal block text-gray-400 hover:text-white mt-1">{{ $crew['title'] }}</a>
                            </div>
                        @endif
                    @endforeach
 ...
 <div class="credits border-b border-gray-800">
        <div class="container mx-auto px-4 py-16">
            ...
            <ul class="list-disc leading-loose pl-5 mt-8">
                @foreach ($actors['credits']['cast'] as $cast)
                    <li>
                        <strong><a href="{{route('movies.show', $cast['id'])}}" class="hover:underline">{{ $cast['title'] }}</a></strong>
                         as {{ $cast['character'] }}
                     </li>
                @endforeach
